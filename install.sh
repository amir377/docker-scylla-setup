...
# Prompt user for ScyllaDB setup details
container_name=$(prompt_user "Enter the container name" "scylla")
network_name=$(prompt_user "Enter the network name" "general")
scylla_cql_port=$(prompt_user "Enter the ScyllaDB CQL port" "9042")
scylla_rest_port=$(prompt_user "Enter the ScyllaDB REST port" "9180")
scylla_user=$(prompt_user "Enter the ScyllaDB username" "admin")
scylla_password=$(prompt_user "Enter the ScyllaDB password" "admin")
allow_host=$(prompt_user "Enter the allowed host" "0.0.0.0")

# Generate the .env file
echo "Creating .env file for ScyllaDB setup..."
cat > .env <<EOL
# ScyllaDB container settings
CONTAINER_NAME=$container_name
NETWORK_NAME=$network_name
SCYLLA_CQL_PORT=$scylla_cql_port
SCYLLA_REST_PORT=$scylla_rest_port
...
if [ -f "docker-compose.example.yaml" ]; then
    sed -e "s/\${CONTAINER_NAME}/$container_name/g" \
        ...
        -e "s/\${SCYLLA_CQL_PORT}/$scylla_cql_port/g" \
        -e "s/\${SCYLLA_REST_PORT}/$scylla_rest_port/g" \
        -e "s/\${ALLOW_HOST}/$allow_host/g" \
        -e "s/\${SCYLLA_USER}/$scylla_user/g" \
        -e "s/\${SCYLLA_PASSWORD}/$scylla_password/g" \
...
echo "Starting Docker Compose with --build for ScyllaDB..."
if docker-compose up -d --build; then
    echo "Checking container status..."
    if [ "$(docker inspect -f '{{.State.Running}}' $container_name)" = "true" ]; then
        echo "ScyllaDB setup is complete and running on ports $scylla_cql_port and $scylla_rest_port."
    else
